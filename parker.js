...
//  (\$\d{1,3})((\,?\b\d{3}\b)?)*((\.\b\d{2}\b)|($))

// ^\$\d+((,\d{3})*)?(\.\d{2})?$

var reg = /^(\$\d+)((\,?\b\d{3}\b)?)*((\.\b\d{2}\b)|($))/gim
...
function testZip (str) {
  ...
  var reg = /(\b\d{5}\b-\b\d{4}\b)|(\b\d{5}\b$)/gim

  return reg.test(str)


//(\b\d{5}\b-\b\d{4}\b)|(\b\d{5}\b[^-])
...
var reg2 = text.replace(/\(/gim, '<a href="').replace(/\)/gim, '">"')

var reg1 = text.replace(/^\[/gim, '<a href=""').replace(/\]/gim, '">"')

// reg = text.replace(/^\[/gim, '<a href="').replace(/\(/gim, "'>")
...
var thisReg = (reg.exec(text))
console.log(thisReg)
  var retString = `<a href="${thisReg[2]}">${thisReg[1]}</a>` 
  // \[([\w\s]*)\]\(([\w\:\/\.]*)\)
